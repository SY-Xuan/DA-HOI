...
from datasets.hico_interaction_qwen import build
from SAP.engine import train_one_epoch, evaluate_hoi
from SAP.models.interaction import InteractionModel
import os
...
def get_args_parser():
    ...
    parser.add_argument('--root_path', default='coco')
...
def main(args):
    ...
    model = InteractionModel(detector_path=args.detector_path, qwen_path=args.qwen_path)
    model.to(device)
    print('****************')
    print(model)
    print('****************')
    ...
    print([n for n, p in model_without_ddp.named_parameters() if p.requires_grad])

    optimizer = torch.optim.AdamW(param_dicts, lr=args.lr,
                                  weight_decay=args.weight_decay)
    lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, args.lr_drop)

    dataset_train = build(root_path=args.root_path, image_set='train', zero_shot_type=args.zero_shot_type, qwen_path=args.qwen_path)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model, data_loader_train, optimizer, device, epoch, args.clip_max_norm)
        lr_scheduler.step()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('SAP training and evaluation script', parents=[get_args_parser()])
