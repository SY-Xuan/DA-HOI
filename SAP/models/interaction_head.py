...
from .attention import TransformerEncoder, TransformerDecoderLayer, TransformerDecoder
...
class HumanObjectMatcher(nn.Module):
    ...
    def forward(self, region_props, memory, device=None):
        if device is None:
        ...
        for i, (rp, mem) in enumerate(zip(region_props, memory)):
            scores, _, boxes, embeds, labels = rp.values()
            ...
            box_pe, c_pe = self.compute_box_pe(boxes, embeds)
            embeds = self.encoder_proj(embeds)
            ...
            pairwise_tokens = self.triplet_decoder(
                ho_q.unsqueeze(1),
                self.decoder_proj(mem.permute(1, 2, 0).reshape(-1, 2048).unsqueeze(1)),
        ...
        match_logits = self.interaction_predictor(pairwise_tokens_collated)

        return logits, match_logits, prior_collated, boxes_h_collated, boxes_o_collated, object_class_collated
