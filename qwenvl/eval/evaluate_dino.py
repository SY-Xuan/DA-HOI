...
object_text = [item['name'] for item in HICO_OBJECTS]
...
def eval_model(args):
    ...
    dino_model_path = "../model_weights/grounding-dino-base"

    dino_processor = AutoProcessor.from_pretrained(dino_model_path)
    dino_model = GroundingDinoForObjectDetection.from_pretrained(dino_model_path).to(device)
    dino_model.eval()
    dino_post_processor = NewGroundingDinoProcessor.from_pretrained(dino_model_path)
    ...
    for line_id, line in enumerate(tqdm(questions)):
        ...
        dino_inputs = dino_processor(images=image, text=object_text, return_tensors="pt").to(device)
        with torch.inference_mode():
            dino_outputs = dino_model(**dino_inputs)
        dino_results = dino_post_processor.post_process_grounded_object_detection_filter(dino_outputs, dino_inputs['input_ids'], box_threshold=0.05)[0]
        ...
        with torch.inference_mode():
            ...
            sap_boxes, sap_labels, merge_box, merge_classes, merge_scores = model.get_grounding_dino_results(
                dino_results['scores'], dino_results['boxes'], dino_results['labels_id'],
            )
