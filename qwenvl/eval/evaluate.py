...
def eval_model(args):
    ...
    for line_id, line in enumerate(tqdm(questions)):
        ...
        detr_image = transform(image).to(dtype=torch.float16, device=device).unsqueeze(0)
        ...
        with torch.inference_mode():
            ...
            sap_boxes, sap_labels, merge_box, merge_classes, merge_scores = model.get_dino_results(
                dino_pixel_values=detr_image.to(dtype=torch.float16, device=device),
            )
